...
if [ ! -f "$CONFIG_PATH" ]; then
  log "❌ Configuration file not found at $CONFIG_PATH"
  exit 1
fi
...
if [ -z "$GITHUB_ORG" ] || [ -z "$GITHUB_TOKEN" ] || [ "$GITHUB_TOKEN" = "YOUR_GITHUB_PERSONAL_ACCESS_TOKEN" ]; then
  log "⚠️ Invalid or default GITHUB_TOKEN/GITHUB_ORG in configuration."
...
log "Checking GitHub organization runs for ${GITHUB_ORG}..."
if [ "$DRY_RUN" = "true" ]; then
  # Mock count for dry-run verification
  QUEUED_COUNT="${MOCK_QUEUED_COUNT:-1}"
  IN_PROGRESS_COUNT="${MOCK_IN_PROGRESS_COUNT:-0}"
  log "[DRY RUN] Mocking ${QUEUED_COUNT} queued run, ${IN_PROGRESS_COUNT} in-progress runs."
else
...
log "Active Jobs in Fleet Queue: ${ACTIVE_JOBS} (Queued: ${QUEUED_COUNT}, In-Progress: ${IN_PROGRESS_COUNT})"
...
while [ "$INDEX" -lt "$HOST_COUNT" ]; do
  ...
  log "Processing host ${HOST_NAME} (${HOST_IP})..."
  ...
  if [ "$PING_STATUS" -eq 0 ]; then
    log "Host ${HOST_NAME} is ONLINE."

    if [ "$ACTIVE_JOBS" -gt 0 ]; then
      log "Active jobs found. Booting/starting container ${CONTAINER_NAME}..."
      if [ "$DRY_RUN" = "true" ]; then
        log "[DRY RUN] ssh -i ${SSH_KEY} ${SSH_USER}@${HOST_IP} 'docker start ${CONTAINER_NAME}'"
      else
        ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no -o ConnectTimeout=5 "${SSH_USER}@${HOST_IP}" "docker start ${CONTAINER_NAME}"
      fi
    else
      log "No active jobs. Ensuring container ${CONTAINER_NAME} is stopped..."
      if [ "$DRY_RUN" = "true" ]; then
        log "[DRY RUN] ssh -i ${SSH_KEY} ${SSH_USER}@${HOST_IP} 'docker stop ${CONTAINER_NAME}'"
      else
      ...
      if [ "$SUSPEND_IDLE" = "true" ]; then
        ...
        if [ "$DRY_RUN" = "true" ]; then
          WHO_COUNT=1
          log "[DRY RUN] Checking active sessions. Mocking WHO_COUNT=1"
        else
        ...
        if [ -n "$WHO_COUNT" ] && [ "$WHO_COUNT" -le 1 ]; then
          log "Host ${HOST_NAME} is idle. Initiating system suspend..."
          if [ "$DRY_RUN" = "true" ]; then
            log "[DRY RUN] ssh -i ${SSH_KEY} ${SSH_USER}@${HOST_IP} 'sudo systemctl suspend'"
          else
            ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no -o ConnectTimeout=5 "${SSH_USER}@${HOST_IP}" "sudo systemctl suspend" >/dev/null 2>&1
          fi
        else
          log "Host ${HOST_NAME} has active user sessions (${WHO_COUNT}). Skipping suspend."
        fi
      fi
    fi
  else
    log "Host ${HOST_NAME} is OFFLINE."

    if [ "$ACTIVE_JOBS" -gt 0 ]; then
      log "Active jobs found! Broadcasting WoL magic packet to ${HOST_MAC}..."
      if [ "$DRY_RUN" = "true" ]; then
        log "[DRY RUN] etherwake ${HOST_MAC}"
      else
        if command -v etherwake >/dev/null 2>&1; then
          ...
          log "❌ Neither etherwake nor wol commands were found on this router."
        fi
      fi
      ...
      BOOT_TIMEOUT=120
      ELAPSED=0
      log "Waiting for host ${HOST_NAME} to boot..."
      while [ "$ELAPSED" -lt "$BOOT_TIMEOUT" ]; do
        ping -c 1 -W 2 "$HOST_IP" >/dev/null 2>&1
        if [ $? -eq 0 ]; then
          log "Host ${HOST_NAME} successfully booted!"
          sleep 5
      ...
      if [ "$ELAPSED" -ge "$BOOT_TIMEOUT" ]; then
        log "❌ Timeout waiting for host ${HOST_NAME} to boot."
      fi
    fi
  fi
...
log "Lifecycle processing complete."
